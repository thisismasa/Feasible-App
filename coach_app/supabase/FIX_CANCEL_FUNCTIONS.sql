-- ============================================================================
-- FIX: Cancel session functions with ANY/ALL array operator error
-- ============================================================================
-- Error: "op ANY/ALL (array) requires operator to yield boolean, code: 42809"
-- Root cause: Incorrect usage of ANY/ALL with arrays
-- ============================================================================
...
FROM pg_proc
WHERE proname LIKE '%cancel%session%'
  AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
ORDER BY proname;

-- ============================================================================
-- FIX #1: cancel_session_with_refund
-- ============================================================================
...
-- ============================================================================
-- FIX #2: cancel_session_with_reason
-- ============================================================================
...
-- WHAT WAS FIXED:
-- ============================================================================
-- 1. Changed: WHERE status = ANY(ARRAY['scheduled', 'confirmed'])
--    To: WHERE status IN ('scheduled', 'confirmed')
--
-- 2. Changed: WHERE status = ALL(ARRAY['active'])
--    To: WHERE status = 'active'
--
-- 3. Added proper error handling with JSON response format
-- 4. Made refund logic work with sessions.package_id = client_packages.id
-- 5. Added NOTICE logs for debugging
-- ============================================================================
...
-- 3. Try cancelling a session - should work now!
-- 4. Check terminal for: "Session cancelled: Refunded package <id>"
-- ============================================================================
